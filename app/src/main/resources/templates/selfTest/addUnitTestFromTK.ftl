...
<form id="pagerForm" method="post" action="${ctx}/unitTest/config?typeid=${typeid}">
...
<div class="pageHeader">
    <form id="questionForm" onsubmit="return navTabSearch(this);" action="${ctx}/unitTest/config" class="required-validate" method="post">
        <div class="searchBar">
...
<div class="pageContent">
    ...
    <#if questionDOs??&&questionDOs?size gt 0>
        <#list questionDOs as questionDO>
            ...
            <div class="panelBar">
                <ul class="toolBar">
                    <#if questionDO.unitFlag! == 0>
                        <li style="float: right"><a class="add" href="javascript:void(0)" id="add${questionDO.getId()}" name="add${questionDO.getId()}" onclick="addToSyncTest(${questionDO.getId()});" ><span id="add">加入单元测试</span></a></li>
                    <#else >
                        <li style="float: right;color: grey"><a href="javascript:return false;" onclick="return false;" ><span id="add" style="color: #CCC;">已加入单元测试</span></a></li>
                    </#if>
