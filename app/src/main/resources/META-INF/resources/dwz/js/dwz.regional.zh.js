...
(function($){
	...
	if ($.validator) {
		$.extend($.validator.messages, {
			...
			chineseAndEnglishAndNumber: "支持中英文、数字录入。",
			chineseAndEnglish: "不允许录入特殊符号和数字。",
			alphanumeric: "字母、数字、下划线",
			lettersonly: "必须是字母",
			phone: "数字、空格、括号",
            chineseAndEnglishAndNumberAndUnderline:"支持中英文,数字,下划线",
            criterionEmail:"请输入正确格式的电子邮件"
		});
	}

    $.validator.addMethod("notChinese", function(value, element) {
        return this.optional(element) || !/[\u4e00-\u9fa5]+$/gi.test(value);
    }, "不能输入中文");
