        ...
        bringBackSuggest: function (args) {
            ...
            $box.find(":input").each(function () {
                ...
                for (var key in args) {
                    ...
                    if (name == inputName) {
                        $input.val(args[key]);
                        if($input.attr("name").indexOf("productName")!=-1){
                            $input.trigger('change');
                            $input.attr("id",$input.attr("name"));
                        }else if($input.attr("name").indexOf("riskName")!=-1){
                            $input.trigger('change');
                        }
                        break
                    }
        ...
        lookup: function () {
            return this.each(function () {
                ...
                $this.click(function (event) {
                    ...
                    var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
                    if(url.indexOf("productConfig/planDef/getEngage?productCode=")>0){
                        var productCode = $("#productCodeValue").val();
                        if(productCode==null||productCode==""){
                            alertMsg.error("请先选择产品信息");
                            return false
                        }
                        url =url + productCode;
                    }
            ...
            return this.each(function () {
                ...
                $tbody.find("a.btnDel").click(function () {
                    ...
                    if ($btnDel.is("[href^=javascript:]")) {
                        if($tbody.attr('id')=='riskList'){
                            if($("#riskList").children("tr").size()==1){
                                alert("至少需要选择一个险种")
                                return
                            }
                        }
                        ...
                        //组合产品。删除险种时 重新查询一下条款
                        if($tbody.attr('id')=='riskList'){
                            for(var i=0;i<$("#riskList").children("tr").size();i++){
                                $("select[name='prpdClassAndPrpdRiskVoList["+i+"].riskCode']").trigger("change");
                            }
                        }
                        initSuffix($tbody);
                        return false
